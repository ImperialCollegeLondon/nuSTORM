...
for i in range(n_tst4):
  npi += 1
  ppi = np.append(ppi, nuTrfLine.GeneratePiMmtm(5.))
  Epi = np.append(Epi, nuTrfLine.CalculateE(ppi[npi-1]))
  tpi = np.append(tpi, nuTrfLine.GenerateTime())
  spi = tpi[npi-1]*nuTrfLine.Calculatev(ppi[npi-1])
  zpi = np.append(zpi, nuTrfLine.Calculatez(spi))
  x, y, xp, yp = nuTrfLine.GenerateTrans(zpi[npi-1])
  xpi  = np.append(xpi, x)
  ypi  = np.append(ypi, y)
  xppi = np.append(xppi, xp)
  yppi = np.append(yppi, yp)
  gamma = Epi[i]/(piCnst.mass()/1000.)
  vpi = nuTrfLine.Calculatev(ppi[npi-1])
  t_dcy_pi = np.append(t_dcy_pi, nuTrfLine.GenerateDcyTime(gamma))
  DcyPt = nuTrfLine.CalculateDcyPt(s=-50.,s_final=0.,t_i=tpi[npi-1],v=vpi,gamma=gamma)
  if DcyPt > -100.:
      s_dcy_pi = np.append(s_dcy_pi, DcyPt)
  t_prod_pi = np.append(t_prod_pi, nuTrfLine.Calculatet(s=-50.,s_final=0.,t_i=tpi[npi-1],v=vpi))
  if npi < 11:
...
print()
print("nuSTORMTrfLineTest:", nuSTORMTrfLineTest, " pion distributions from scratch.")
p0 = 5.
runNum = 101
eventNum = 0
n_tst5 = n_tst
...
eH.outFile("Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst5.root")
eH.rootStructure()
...
for eventNum in range(n_tst5):
    ...
    eH = nuTrfLine.GeneratePion(eventHist=eH,runNum=runNum,eventNum=eventNum,weight=weight,p0=p0)
    ...
    if pi_prod.run() != -1:
        ...
        if pi_dcy != None or mu != None or nu != None:
...
nuSTORMTrfLineTest = 6
print()
print("nuSTORMTrfLineTest:", nuSTORMTrfLineTest, " pion distributions from parameter input.")
runNum = 102
eventNum = 0
n_tst6 = n_tst4
weight = 1/n_tst6
...
eH = eventHistory.eventHistory()
eH.outFile("Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst6.root")
eH.rootStructure()
...
for eventNum in range(n_tst6):
    ...
    eH = nuTrfLine.GeneratePion(eventHist=eH,x=xpi[eventNum],y=ypi[eventNum],px=px,py=py,pz=pz,t=tpi[eventNum],weight=weight,runNum=runNum,eventNum=eventNum)
    ...
    if pi_target.traceSpace().s() != -50.:
        raise Exception("ERROR: s for pion at target is not -50 m!")
    ...
    if pi_prod.run() != -1:
        ...
        if pi_prod.traceSpace().s() != 0.:
            raise Exception("ERROR: s for pion at production straight entrance is not 0 m!")
    ...
    ## Checks if pion has decayed in the transfer line
        ...
        P_mu_rf, P_numu_rf = nuTrfLine.BoostBack2RestFrame(PPi=ppi[eventNum],P_mu=mu.p(),P_numu=nu.p())
        ...
        if (pi_dcy.traceSpace().s() >= 0. or pi_dcy.traceSpace().s() <= -50.):
            print("s = ",pi_dcy.traceSpace().s())
            raise Exception("ERROR: s for pion at decay point does not lie within range (-50m,0m)!")
...
nuSTORMTrfLineTest = 7
print()
print("nuSTORMTrfLineTest:", nuSTORMTrfLineTest, " boost back method.")
...
P_mu_nS, P_numu_nS = nuTrfLine.Boost2nuSTORM(PPi,P_mu,P_numu)
P_mu_rf, P_numu_rf = nuTrfLine.BoostBack2RestFrame(PPi,P_mu_nS,P_numu_nS)
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_p.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_E.pdf')
plt.close()

n, bins, patches = plt.hist(tpi, bins=50, color='y', range=(0.,2.*10**(-9)))
plt.xlabel('t (s)')
plt.ylabel('Entries')
plt.title('t distribution')
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_t.pdf')
plt.close()

n, bins, patches = plt.hist(-zpi-50., bins=50, color='y', range=(-50.-max(zpi),-50.))
plt.xlabel('z (m)')
plt.ylabel('Entries')
plt.title('z distribution at t=0')
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_z.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_x.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_y.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_xp.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_yp.pdf')
plt.close()

n, bins, patches = plt.hist(t_dcy_pi, bins=50,color='y',range=(0.,0.8*10**(-5)),log=True) #,range=(0.,0.6*10**(-5))
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_sdec.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/nuSTORMTrfLineTst_4_tprod.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/0_target/nuSTORMTrfLineTst_5_piE.pdf')
plt.close()

n, bins, patches = plt.hist(t_pi_t, bins=50, color='y', range=(0.,2.*10**(-9)))
plt.xlabel('t (s)')
plt.ylabel('Entries')
plt.title('t distribution of pion at target')
plt.savefig('Scratch/nuSTORMTrfLine/0_target/nuSTORMTrfLineTst_5_piT.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/0_target/nuSTORMTrfLineTst_5_piX.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/0_target/nuSTORMTrfLineTst_5_piY.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/0_target/nuSTORMTrfLineTst_5_piXp.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/0_target/nuSTORMTrfLineTst_5_piYp.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/1_productionStraight/nuSTORMTrfLineTst_5_piT.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_piT.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_piS.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_muE.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_muX.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_muY.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_muXp.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_muYp.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_nuE.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_nuX.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_nuY.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_nuXp.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_5_nuYp.pdf')
plt.close()


#For nuSTORMTrfLineTest = 6
n, bins, patches = plt.hist(E_pi_trgt_tst6, bins=50, color='y', range=(4.0,6.0))
plt.xlabel('Energy (GeV)')
plt.ylabel('Entries')
plt.title('Pion energy distribution')
plt.savefig('Scratch/nuSTORMTrfLine/0_target/nuSTORMTrfLineTst_6_piE.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('t distribution of pion at production straight entrance')
plt.savefig('Scratch/nuSTORMTrfLine/1_productionStraight/nuSTORMTrfLineTst_6_piT.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('Decay time distribution')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_piT.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_piZ.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('Energy distribution of muon at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muE.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('x distribution of muon at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muX.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('y distribution of muon at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muY.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('x^prime distribution of muon at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muXp.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('y^prime distribution of muon at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muYp.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('Energy distribution of PiFlashNu at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuE.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('x distribution of PiFlashNu at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuX.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('y distribution of PiFlashNu at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuY.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('x^prime distribution of PiFlashNu at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuXp.pdf')
plt.close()
...
plt.ylabel('Entries')
plt.title('y^prime distribution of PiFlashNu at production point')
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuYp.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muE_rf.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muXp_rf.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_muYp_rf.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuE_rf.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuXp_rf.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_nuYp_rf.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_cosTheta_rf.pdf')
plt.close()
...
plt.savefig('Scratch/nuSTORMTrfLine/2_pionDecay/nuSTORMTrfLineTst_6_phi_rf_tst.pdf')
plt.close()
